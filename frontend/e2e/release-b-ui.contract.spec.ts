...
test('platform templates list and assign template to tenant', async ({ page }) => {
  ...
  await page.route('**/api/v1/platform/**', async (route) => {
    ...
    if (pathname.endsWith('/catalog-templates') && route.request().method() === 'GET') {
      return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify([{ id: 'tpl-1', verticalId: 'vert-1', name: 'Template Uno', version: '1.0', isActive: true, createdAtUtc: '2026-01-01T00:00:00Z', updatedAtUtc: '2026-01-01T00:00:00Z' }]) });
    }
...
test('tenant overrides and availability send expected payloads', async ({ page }) => {
  ...
  await page.route('**/api/v1/pos/catalog/snapshot**', async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({ storeId: 'store-e2e', timeZoneId: 'America/Mexico_City', generatedAtUtc: '2026-01-01T00:00:00Z', catalogVersion: 'v1', etagSeed: 's1', categories: [], products: [{ id: 'product-1', externalCode: null, name: 'Producto', categoryId: 'c1', subcategoryName: null, basePrice: 10, isActive: true, isAvailable: true, customizationSchemaId: null }], optionSets: [], optionItems: [], schemas: [], selectionGroups: [], extras: [], includedItems: [], overrides: [], versionStamp: 'v1' }),
    });
  });
  ...
  await page.route('**/api/v1/pos/admin/**', async (route) => {
    ...
    if (pathname.endsWith('/products') && route.request().method() === 'GET') {
      return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify([{ id: 'product-1', externalCode: null, name: 'Producto', categoryId: 'c1', subcategoryName: null, basePrice: 10, isActive: true, isAvailable: true, customizationSchemaId: null }]) });
    }
    if (pathname.endsWith('/categories') && route.request().method() === 'GET') {
      return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify([{ id: 'c1', name: 'Cat', sortOrder: 1, isActive: true }]) });
    }
