...
test('store overrides UI can disable and clear explicit override by store', async ({ page }) => {
  ...
  await page.route('**/api/v1/pos/**', async (route) => {
    ...
    if (pathname.endsWith('/admin/extras') && request.method() === 'GET') {
      return route.fulfill({
        ...
        contentType: 'application/json',
        body: JSON.stringify([
          { id: 'extra-1', name: 'Shot', price: 2, isActive: true, isAvailable: true },
        ]),
      });
    }
    ...
    if (pathname.endsWith('/admin/option-sets') && request.method() === 'GET') {
      return route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify([{ id: 'set-1', name: 'Leches', isActive: true }]),
      });
    }
  ...
  await storeSelect.fill('store-e2e');
  ...
  await expect(page.getByTestId('store-override-state-Product-product-1')).toContainText(
    'Disabled',
  );
...
test('inventory lite UI updates stock for products and extras using release C endpoint', async ({
  page,
}) => {
...
test('pos badge contract shows disabled reasons and disabled controls', async ({ page }) => {
          ...
          products: [
            {
              ...
              isAvailable: false,
              customizationSchemaId: null,
              availabilityReason: 'DisabledByStore',
            },
            {
  ...
  await expect(page.getByTestId('availability-badge-Product-product-store-disabled')).toContainText(
    'DisabledByStore',
  );
  await expect(page.getByTestId('availability-badge-Product-product-out-stock')).toContainText(
    'OutOfStock',
  );
