...
describe('PosInventoryAdminApiService', () => {
  it('builds expected urls and payloads for inventory release C endpoints', async () => {
    ...
    await service.validateInventoryBatchAdjustmentV2({ storeId: 'store-1', reasonCode: 'Correction', clientOperationId: 'batch-2', items: [{ itemType: 'Product', itemId: 'product-1', operationType: 'Delta', quantityDelta: -1 }] });
    ...
    expect(calls).toContainEqual({ method: 'post', path: '/v2/pos/inventory/adjustments/batch/validate', body: { storeId: 'store-1', reasonCode: 'Correction', clientOperationId: 'batch-2', items: [{ itemType: 'Product', itemId: 'product-1', operationType: 'Delta', quantityDelta: -1 }] } });
