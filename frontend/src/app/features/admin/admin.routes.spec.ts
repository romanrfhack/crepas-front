...
describe('adminRoutes', () => {
  it('should include lazy POS catalog route protected for AdminStore and Manager roles', () => {
    const route = adminRoutes.find((item) => item.path === 'pos/catalog');
    expect(route).toBeDefined();
    expect(route?.loadChildren).toBeDefined();
    expect(route?.data?.['roles']).toEqual(['AdminStore', 'Manager', 'TenantAdmin', 'SuperAdmin']);
  ...
  it('should block access to admin route for non-admin users', () => {
    ...
    const route = { path: 'pos/catalog', data: { roles: ['AdminStore', 'Manager', 'TenantAdmin', 'SuperAdmin'] } };
