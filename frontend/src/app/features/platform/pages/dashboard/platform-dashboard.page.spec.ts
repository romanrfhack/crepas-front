...
describe('PlatformDashboardPage', () => {
  ...
  it('clicking alert opens drilldown and renders items', async () => {
    const host = fixture.nativeElement as HTMLElement;
    host.querySelector('[data-testid="platform-alert-drilldown-open-TENANT_WITHOUT_TEMPLATE"]')?.dispatchEvent(new Event('click'));
    await fixture.whenStable();
    fixture.detectChanges();
    ...
    expect(host.querySelector('[data-testid="platform-alert-drilldown-row-0"]')?.textContent).toContain('Tenant sin template');
  });

  it('clicking tenant opens tenant overview', async () => {
    const host = fixture.nativeElement as HTMLElement;
    host.querySelector('[data-testid="platform-tenant-overview-open-tenant-1"]')?.dispatchEvent(new Event('click'));
    await fixture.whenStable();
    fixture.detectChanges();
    ...
    expect(host.querySelector('[data-testid="platform-tenant-overview-metric-tenantName"]')?.textContent).toContain('Tenant 1');
  });

  it('clicking stockout hotspot opens stockout details and filter apply sends request', async () => {
    const host = fixture.nativeElement as HTMLElement;
    host.querySelector('[data-testid="platform-store-stockout-open-store-2"]')?.dispatchEvent(new Event('click'));
    await fixture.whenStable();
    fixture.detectChanges();

    expect(service.getStoreStockoutDetails).toHaveBeenCalledWith('store-2', expect.objectContaining({ mode: 'out-of-stock' }));
  ...
  it('shows stable invalid alert code error for 400 responses', async () => {
    ...
    const host = fixture.nativeElement as HTMLElement;
    expect(host.querySelector('[data-testid="platform-alert-drilldown-error"]')?.textContent).toContain('Código de alerta inválido o no soportado.');
  });

  it('shows drilldown empty and error states', async () => {
    service.getAlertDrilldown.mockResolvedValueOnce({ code: 'STORE_WITHOUT_ADMINSTORE', items: [] });
