...
public interface IPosCatalogService
{
    ...
    Task<PagedInventoryBalancesDto> GetInventoryBalancesV2Async(Guid storeId, string? query, Guid? categoryId, bool? tracked, decimal? onHandMin, decimal? onHandMax, int page, int pageSize, CancellationToken ct);
    Task<IReadOnlyList<InventoryBalanceRowDto>> GetInventoryBalancesV2ExportAsync(Guid storeId, string? query, Guid? categoryId, bool? tracked, decimal? onHandMin, decimal? onHandMax, int maxRows, CancellationToken ct);
