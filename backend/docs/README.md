...
1. Define `IdentitySeed:AdminEmail` y `IdentitySeed:AdminPassword` por `user-secrets` o variables del proceso.
    - Ejemplo:
        ...
        ```
2. Arranca la API en `Development` y el usuario se crea/asigna al rol `AdminStore`.

> Si faltan `AdminEmail` o `AdminPassword`, el seed del admin se omite y solo se crean los roles por defecto (`AdminStore`, `User`, `Manager`, `Collector`, `Cashier`, `TenantAdmin`, `SuperAdmin`).
