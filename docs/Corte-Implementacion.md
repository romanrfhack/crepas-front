...
| A2. Admin usuarios (list/get/roles/lock)      | DONE    | `AdminUsersController` con `GET /admin/users`, `GET /admin/users/options`, `GET /admin/users/{id}`, `POST /admin/users`, `PUT /roles`, `PUT /admin/users/{id}`, `POST /lock`, ruta compat `PUT /lock` y `POST /temporary-password`; policy vigente de administración de usuarios. | `AdminUsersAuthorizationHardeningIntegrationTests`, `AdminUsersCreateIntegrationTests`, `AdminUsersUpdateIntegrationTests`, `AdminUsersTemporaryPasswordIntegrationTests`, unit/E2E frontend. | Contrato vigente en `docs/admin/user-administration.md`; backlog separado: hardening de `AdminRolesController`.                         |
...
- Regla: `/api/v1/admin/users*` aplica policy de administración de usuarios, scope tenant/store, jerarquía estricta y opciones seguras desde `/api/v1/admin/users/options`; `/api/v1/admin/roles` queda como superficie separada/backlog de hardening.
- Dónde: `AdminUsersController`, `AdminRolesController`.
- Falta E2E/backlog: endurecer o restringir `AdminRolesController`; mantener contrato de `/admin/users` alineado con `docs/admin/user-administration.md`.
...
5. **SEC-001 Matriz de autorización por endpoint crítico**
   - Descripción: tests negativos 401/403 para políticas vigentes de Admin Users, PosOperator y PosAdmin.
